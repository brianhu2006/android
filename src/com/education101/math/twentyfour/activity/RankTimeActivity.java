...
public class RankTimeActivity extends ListActivity {
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		Log.i(TAG, "排行榜固定时间模式,菜单Item:"+item.getItemId()+"-"+item.getTitle());
	...
	class LoadDataTask extends AsyncTask<Integer,Integer,Integer>{
		...
		protected Integer doInBackground(Integer... params) {
	        ...
	        titleMap.put("rank_time_or_count", "正确题数");  
	        ...
	        for(int i=1;i<=gameList.size();i++){  
	            ...
	            map.put("rank_time_or_count", game.getGameRightTime()+"题");  
	            map.put("rank_date", DateUtil.dateToString(game.getGameDate()));  
	            listItem.add(map);  
	        } 
	        //生成适配器的Item和动态数组对应的元素  
	        listItemAdapter = new SimpleAdapter(RankTimeActivity.this,listItem,//数据源   
		...
		protected void onPostExecute(Integer result) {			
			if(result == -1)
				Toast.makeText(RankTimeActivity.this, "还没有人上榜哦.", Toast.LENGTH_LONG).show();
